...
export const SOURCE_CATALOG: ConnectorMeta[] = [
  ...
  // ── Communication ──────────────────────────────────────────────────
  {
    ...
    steps: [
      {
        label: 'Make sure 2-Step Verification is enabled, then generate a 16-character App Password (Mail / Other / "TokyoOS"). Paste it below \u2014 spaces are fine, and use the app password, not your regular Gmail password.',
  ...
  },
  {
    ...
    steps: [
      {
        label: 'Go to api.slack.com/apps and click "Create New App" → choose "From scratch". Name it "TokyoOS" and pick your workspace',
      ...
      {
        label: '(Optional) Set the app icon: in the left sidebar click "Basic Information" → scroll to "Display Information" → upload the TokyoOS logo',
        url: 'https://github.com/open-jarvis/TokyoOS/blob/main/assets/openjarvis-slack-icon.jpg',
  ...
  },
  {
    ...
    steps: [
      {
        label: 'Go to notion.so/profile/integrations → click "+ New integration". Name it "TokyoOS", select your workspace, and click Submit',
      ...
      {
        label: 'To share ALL your pages at once: open any top-level page → click "..." (top right) → "Connections" → "Add connections" → search "TokyoOS" → click it. This shares the page and all its sub-pages. Repeat for each top-level page, or share your entire workspace by doing this on every root page',
      },
      {
  ...
  },
  {
    ...
    steps: [
      ...
      {
        label: 'Click the "+" button at the bottom of the list. Navigate to Applications → Utilities → select "Terminal.app" (or iTerm2/Warp if you use those). If you\'re using the desktop app, also add "TokyoOS.app" from Applications',
      },
      {
        label: 'Toggle the switch ON next to each app you added. Close and reopen your terminal (or restart TokyoOS). iMessage data will be detected automatically — no credentials needed',
      },
    ],
  },
  // ── Documents ──────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      {
        label: 'Paste the full path below. TokyoOS will index all .md files in the vault',
      },
    ],
    inputFields: [
  ...
  },
  {
    ...
    steps: [
      {
        label: 'Go to Google Cloud Console → create a new project (or select an existing one). Give it any name (e.g. "TokyoOS")',
  ...
  },
  {
    ...
    steps: [
      ...
      {
        label: 'Toggle the switch ON next to each app you added. Close and reopen your terminal (or restart TokyoOS). Apple Notes will be detected automatically — no credentials needed',
      },
    ],
  },
  {
    ...
    steps: [
      ...
      {
        label: 'Toggle the switch ON next to each app you added. Close and reopen your terminal (or restart TokyoOS). Apple Contacts will be detected automatically — no credentials needed',
      },
    ],
  },
  {
    ...
    steps: [
      {
        label: 'Go to the Azure Portal → App Registrations → click "+ New registration". Name it "TokyoOS", select "Accounts in this organizational directory only", and click Register',
  ...
  },
  {
    ...
    steps: [
      {
        label: 'Go to the Dropbox App Console and click "Create app". Choose "Scoped access" → "Full Dropbox" → give it a name (e.g. "TokyoOS") → click "Create app"',
